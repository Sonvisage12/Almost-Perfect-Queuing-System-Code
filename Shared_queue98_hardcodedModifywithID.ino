...
// ... [includes and declarations remain unchanged]
...
void onDataRecv(const esp_now_recv_info_t *recvInfo, const uint8_t *incomingData, int len) {
    ...
    if (isArrivalNode) {
        ...
        if (strcmp(item.uid, "REQ_NEXT") == 0) {
            ...
            // Find which doctor node requested based on MAC
            int doctorNodeID = -1;
            for (int i = 0; i < 4; i++) {
                if (memcmp(mac, doctorMACs[i], 6) == 0) {
                    doctorNodeID = i + 1;  // Node IDs are 1-based
                    break;
                }
            }
            ...
            if (!sharedQueue.empty()) {
                ...
                sendItem.node = doctorNodeID;  // 🔥 Include Doctor Node ID
                ...
                zeroItem.number = 0;
                zeroItem.node = doctorNodeID;  // 🔥 Include Doctor Node ID in 'NO_PATIENT'
